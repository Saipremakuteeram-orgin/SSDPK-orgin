...
(function() {
  ...
  function upgradeRedesign() {
    ...
    document.querySelectorAll('a[href$=".html"], a[href*=".html?"]').forEach(function(link) {
      ...
      if (href) {
        ...
        link.setAttribute('href', base + '?v=1.0.8');
      }
    });
  }
  ...
  // Floating mute/unmute button on all pages
  // Music starts on first user interaction (browser autoplay policy)
  // Mute preference persists across all pages via localStorage
  // ══════════════════════════════════════════════════════════════════════════
  (function setupAudioPlayer() {
    var MUTE_KEY = 'sspk_music_muted';
    var isMuted = localStorage.getItem(MUTE_KEY) === 'true';
    ...
    style.textContent = [
      '@keyframes sspkPulse {',
      '  0%,100% { box-shadow: 0 4px 24px rgba(192,122,62,0.25), 0 0 0 0 rgba(192,122,62,0.3); }',
      '  50% { box-shadow: 0 4px 32px rgba(192,122,62,0.45), 0 0 0 8px rgba(192,122,62,0); }',
      '}',
      '@keyframes sspkMusicBtnIn {',
      ...
      '  gap: 7px;',
      '  padding: 10px 18px;',
      '  border: 1.5px solid rgba(192,122,62,0.4);',
      '  border-radius: 999px;',
      ...
      '  cursor: pointer;',
      '  box-shadow: 0 4px 24px rgba(192,122,62,0.25);',
      '  transition: all 0.25s cubic-bezier(0.34,1.56,0.64,1);',
      '  animation: sspkMusicBtnIn 0.5s ease 0.8s both;',
      '  user-select: none;',
      '  -webkit-tap-highlight-color: transparent;',
      '}',
      '#sspk-music-btn:hover {',
      '  border-color: rgba(192,122,62,0.75);',
      '  background: rgba(192,122,62,0.18);',
      '  transform: translateY(-2px) scale(1.04);',
      '  box-shadow: 0 8px 32px rgba(192,122,62,0.35);',
      '}',
      '#sspk-music-btn:active {',
      '  transform: scale(0.96);',
      '}',
      '#sspk-music-btn.playing {',
      '  animation: sspkMusicBtnIn 0.5s ease 0.8s both, sspkPulse 2.5s ease-in-out 1.3s infinite;',
      '}',
      '#sspk-music-btn .music-icon {',
      '  font-size: 16px;',
      '  line-height: 1;',
      '  transition: transform 0.2s;',
      '}',
      '#sspk-music-btn:hover .music-icon {',
      '  transform: rotate(-8deg) scale(1.15);',
      '}',
      '#sspk-music-btn .music-label {',
      '  font-size: 12px;',
      '  white-space: nowrap;',
      '}',
      /* Mobile: compact icon-only circle, centered at bottom */
    ...
    // Sources — primary local, fallback external
    var src1 = document.createElement('source');
    src1.src = 'audio/melody.mp3';
    ...
    // Start on first user interaction (browser autoplay rule)
    var startEvents = ['click', 'scroll', 'keydown', 'touchstart'];
    function onFirstInteraction() {
      startAudio();
      startEvents.forEach(function(ev) {
        document.removeEventListener(ev, onFirstInteraction, { passive: true });
      });
    }
    startEvents.forEach(function(ev) {
    ...
    btn.addEventListener('click', function(e) {
      ...
      if (!hasStarted) {
        // First interaction is the button click
        hasStarted = true;
        isMuted = false;
        audio.load();
